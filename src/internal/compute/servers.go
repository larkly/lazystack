...
// EvacuateServer evacuates a server from a failed host. Returns the new
// admin password generated by Nova when no target host is given.
func EvacuateServer(ctx context.Context, client *gophercloud.ServiceClient, id, targetHost string, onSharedStorage bool) (string, error) {
	...
	adminPass, err := r.ExtractAdminPass()
	if err != nil {
		shared.Debugf("[compute] extract evacuate admin pass for %s: %v", id, err)
	}
