...
func (m *Model) setSizeAllModals(w, h int) {
	...
	m.imageEdit.SetSize(w, h)
	m.imageCreate.SetSize(w, h)
	m.imageDownload.SetSize(w, h)
	m.lbCreate.SetSize(w, h)
	m.lbListenerCreate.SetSize(w, h)
	m.lbPoolCreate.SetSize(w, h)
	m.lbMemberCreate.SetSize(w, h)
	m.lbMonitorCreate.SetSize(w, h)
